# zkML Inference with SP1

Perform verifiable ML inference tasks (e.g., image classification) with [SP1](https://github.com/succinctlabs/sp1).

This demo project contains an SP1 program that receives an input image (JPEG, PNG, etc.) and outputs:

  1. An image class prediction using a simple MLP (multi-layer perceptron) model.
  2. An [IPFS content identifier](https://docs.ipfs.tech/concepts/content-addressing/) (CID).

The [Succinct](https://succinct.xyz/) prover generates a cryptographic proof that binds the class
prediction to the IPFS CID. This proves that an IPFS image (e.g., an IPFS-backed NFT) has a certain
class prediction. Moreover, the proof can be verified onchain by an EVM app.

## What's different about this project?

While there are other [awesome projects](https://github.com/worldcoin/awesome-zkml?tab=readme-ov-file#codebases) that
perform ML inference in ZK proofs, this approach doesn't require designing or using ZK circuits
explicitly. Instead, it uses a general-purpose programming language (Rust) and relies on SP1 to
generate the ZK proofs.

_**If you can write Rust, you can write ZKP programs!**_

## Application

This project enables basic computer vision tasks such as classification, recognition, etc.
on IPFS-backed NFT images with onchain result verification. It only implements a simple
image classifier based on the [Fashion MNIST](https://github.com/zalandoresearch/fashion-mnist)
dataset, but other tasks can be implemented similarly.

The IPFS CID acts as both a serving URI (e.g., what to display) and a commitment of the image
content. An onchain contract must match the NFT's `tokenURI()` property (e.g. `ipfs://...`) with
the proof.

SP1 generates cryptographic proofs that are small in size and cheap to verify onchain with
[`SP1Verifier`](https://github.com/succinctlabs/sp1-contracts/tree/main).

![Diagram](diagram.png)

_Example use cases: perceptual rarity score, NSFW filter, verifiable branding/watermark, verifiable
style, gate access to dog PFPs only, etc._

## Model

The project uses a simple MLP (multi-layer perceptron) trained on the [Fashion MNIST](https://github.com/zalandoresearch/fashion-mnist)
dataset. It classifies images into [10 classes](https://github.com/zalandoresearch/fashion-mnist?tab=readme-ov-file#labels).

For simplicity, the project uses pre-trained model weights from [Hugging Face](https://huggingface.co/sadhaklal/mlp-fashion-mnist/tree/main).
Training the model is out of scope.

## Limitations

The toy model used here is inadequate for real-world applications. However, implementing a larger
and more capable vision model in this way might prove impractical.

It's recommended to use Succinct's [Prover Network](https://docs.succinct.xyz/generating-proofs/prover-network.html)
to generate proofs, as generating proofs locally with commodity hardware is rather slow.

The IPFS CID must be generated with the exact specifications: v0 + DAG-PB + SHA2-256.

## Requirements

- [Rust](https://rustup.rs/)
- [SP1](https://succinctlabs.github.io/sp1/getting-started/install.html)

## How to run

### Execute (without generating a proof)

To run the program without generating a proof:

```sh
export RUST_LOG=info

cargo run --release -- execute --input-image examples/sneaker_0.jpg
```

This command executes the SP1 program and displays its public output.

Example output:

```text
PublicValuesStruct {
    ipfsUri: "ipfs://QmUEDCvfPn59SKP6Ub4ynQmHsZy5FwHXB8fypqV8ydkFdU",
    label: 7,
}
```

The script doesn't upload to IPFS, and uploading isn't necessary to generate the proof. The
content identifier (CID) is derived from the image file. If anyone uploads the file to IPFS
with the same specs, the CID will be the same.

### Generate a proof

```sh
export RUST_LOG=info
export SP1_PRIVATE_KEY=0x...

SP1_PROVER=network cargo run --release -- prove --input-image examples/sneaker_0.jpg
```

### Generate an EVM-compatible proof

Generate a PLONK proof that can be verified onchain:

```sh
SP1_PROVER=network cargo run --release -- evm --input-image examples/sneaker_0.jpg
```

This command outputs a fixture that can be used to test the verification of SP1 zkVM proofs inside
Solidity. Copy the fixture into your Solidity project; e.g., [SP1/Foundry template](https://github.com/succinctlabs/sp1-project-template/tree/main/contracts).

Example output:

```text
{
  "ipfsUri": "ipfs://QmUEDCvfPn59SKP6Ub4ynQmHsZy5FwHXB8fypqV8ydkFdU",
  "label": 7,
  "programVkey": "0x006c7912eb97da386a236d3498aa13dda77bff1b6f2ac5476290fe2f5f49131d",
  "publicValues": "",
  "proof": ""
}
```

## Workspace

- `classifier`: implements MLP
- `classifier-io`: preprocesses input image and generates IPFS CID
- `program`: ZKP SP1 program
- `program-tests`: integration tests for `program`
- `script`: CLI entry point to execute and prove `program`
- `examples`: example input images
- `notebooks`: one-off Jupyter notebooks to export the model weights and inspect inputs
